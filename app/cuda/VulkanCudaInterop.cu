#include "include/app/VulkanCudaInterop.h"
...
__global__ void copyKernel(cudaSurfaceObject_t *dstSurfMipMapArray,
                           cudaTextureObject_t textureMipMapInput, size_t baseWidth,
                           size_t baseHeight) {
...
__global__ void blurKernel(cudaSurfaceObject_t *dstSurfMipMapArray,
                           cudaSurfaceObject_t *srcSurfMipMapArray, size_t baseWidth,
                           size_t baseHeight, int samples, int boundary) {
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void gaussianBlurHorizontal(cudaSurfaceObject_t *intermediateSurf,
                                       cudaSurfaceObject_t *srcSurf, size_t width, size_t height,
...
__global__ void gaussianBlurVertical(cudaSurfaceObject_t *dstSurf,
                                     cudaSurfaceObject_t *intermediateSurf, size_t width,
                                     size_t height, float sigma, int boundary) {
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void sobelKernel(cudaSurfaceObject_t *dstSurfMipMapArray,
                            cudaSurfaceObject_t *srcSurfMipMapArray, size_t baseWidth,
                            size_t baseHeight, int boundary) {
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void sepiaKernel(cudaSurfaceObject_t *dstSurfMipMapArray,
                            cudaSurfaceObject_t *srcSurfMipMapArray, size_t baseWidth,
                            size_t baseHeight, int boundary) {
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void hueRotationKernel(cudaSurfaceObject_t *dstSurfMipMapArray,
                                  cudaSurfaceObject_t *srcSurfMipMapArray, size_t baseWidth,
                                  size_t baseHeight, float hueShift) {
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void sharpenKernel(cudaSurfaceObject_t *dstSurfMipMapArray,
                              cudaSurfaceObject_t *srcSurfMipMapArray, size_t baseWidth,
                              size_t baseHeight, int boundary) {
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void embossKernel(cudaSurfaceObject_t *dstSurfMipMapArray,
                             cudaSurfaceObject_t *srcSurfMipMapArray, size_t baseWidth,
                             size_t baseHeight, int boundary) {
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void BoostColorKernel(cudaSurfaceObject_t *dstSurfMipMapArray,
                                 cudaSurfaceObject_t *srcSurfMipMapArray, size_t baseWidth,
                                 size_t baseHeight, int boundary) {
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (x < baseWidth && y < baseHeight) {
        if (x > boundary) {
            ...
            unsigned int pix = surf2Dread<unsigned int>(srcSurfMipMapArray[0], 4 * x, y);
            float4 col = rgbaIntToFloat(pix);
            // Boost the color values

            float gray = 0.2989f * col.x + 0.5870f * col.y + 0.1140f * col.z;

            col.x = gray + 1.5f * (col.x - gray);
            col.y = gray + 1.5f * (col.y - gray);
            col.z = gray + 1.5f * (col.z - gray);
...
__global__ void hdrKernel(cudaSurfaceObject_t *dstSurfMipMapArray,
                          cudaSurfaceObject_t *srcSurfMipMapArray, size_t baseWidth,
                          size_t baseHeight, int boundary) {
    unsigned int x = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (x < baseWidth && y < baseHeight) {
        if (x > boundary) {
            ...
            float3 hsv = rgbToHsv(make_float3(col.x, col.y, col.z));
            // Increase the value
            hsv.z = min(hsv.z * 1.5f, 1.0f);
            // Convert back to RGB
            float3 rgb = hsvToRgb(hsv);
            float4 hdrCol = make_float4(rgb.x, rgb.y, rgb.z, col.w);
            unsigned int dataB = rgbaFloatToInt(hdrCol);
            surf2Dwrite(dataB, dstSurfMipMapArray[0], 4 * x, y);
        }
    }
}
...
void VulkanCudaInterop::CudaUpdateVkImage() {
    ...
    dim3 dimBlock(nthreads, nthreads,
                  1); // dimBlock.x * dimBlock.y * dimBlock.z <= 1024
    dim3 dimGrid((uint32_t)ceil(float(imageWidth) / dimBlock.x),
                 (uint32_t)ceil(float(imageHeight) / dimBlock.y), 1);

    copyKernel<<<dimGrid, dimBlock>>>(d_surfaceObjectListTemp, textureObjMipMapInput, imageWidth,
                                      imageHeight);
    // gaussianBlurHorizontal<<<dimGrid, dimBlock>>>(d_surfaceObjectListTemp, d_surfaceObjectList,
    //                                               imageWidth, imageHeight, 10, boundary);
    // gaussianBlurVertical<<<dimGrid, dimBlock>>>(d_surfaceObjectList, d_surfaceObjectListTemp,
    //                                             imageWidth, imageHeight, 10, boundary);

    // blurKernel<<<dimGrid, dimBlock, 0, streamToRun>>>(d_surfaceObjectList, d_surfaceObjectListTemp,
    //                                                   imageWidth, imageHeight, filter_radius,
    //                                                   boundary);

    // sobelKernel<<<dimGrid, dimBlock, 0, streamToRun>>>(d_surfaceObjectList, d_surfaceObjectListTemp,
    //                                                    imageWidth, imageHeight, boundary);

    // sepiaKernel<<<dimGrid, dimBlock, 0, streamToRun>>>(d_surfaceObjectList, d_surfaceObjectListTemp,
    //                                                    imageWidth, imageHeight, boundary);

    // hueRotationKernel<<<dimGrid, dimBlock, 0, streamToRun>>>(
    //     d_surfaceObjectList, d_surfaceObjectListTemp, imageWidth, imageHeight,
    //     float(boundary) / WIDTH * 3.141592f);

    // embossKernel<<<dimGrid, dimBlock, 0, streamToRun>>>(
    //     d_surfaceObjectList, d_surfaceObjectListTemp, imageWidth, imageHeight, boundary);

    //BoostColorKernel<<<dimGrid, dimBlock, 0, streamToRun>>>(
    //    d_surfaceObjectList, d_surfaceObjectListTemp, imageWidth, imageHeight, boundary);

    hdrKernel<<<dimGrid, dimBlock, 0, streamToRun>>>(
        d_surfaceObjectList, d_surfaceObjectListTemp, imageWidth, imageHeight, boundary);
